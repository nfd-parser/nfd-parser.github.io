...
本指南提供 NFD Parser 服务的完整配置说明和管理工具。

## 配置文件概述

NFD Parser 使用 YAML 格式的配置文件进行服务配置管理：

- `app-dev.yml` - 主要服务配置
- `server-proxy.yml` - 反向代理服务配置
...
1. **YAML 语法检查** - 确保配置文件符合 YAML 格式规范
2. **配置项验证** - 检查必需的配置项是否存在
3. **类型检查** - 验证配置值的数据类型是否正确
...
1. **敏感信息保护** - 数据库密码等敏感信息应使用环境变量或加密存储
